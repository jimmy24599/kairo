...
class Agent {
  ...
  async processRequest(userInput, projectFiles = [], ws = null, chatId = null, projectName = null) {
    ...
    const todoList = await this.generateTodoList(userInput, projectAnalysis, ws)
  ...
  async generateTodoList(userInput, projectAnalysis, ws = null) {
    ...
    const systemPrompt = `You are an expert project manager and developer. Based on the user's request and project analysis, create a detailed todo list.
...
CRITICAL RULES:
1. Create 3-6 specific, actionable tasks
2. Tasks should be ordered logically (dependencies first)
3. Each task should be clear and specific
4. Consider the project type and framework
5. Focus on what the user actually requested
6. Use proper Next.js App Router conventions if it's a Next.js project
...
[
  ...
  },
  {
    ...
    try {
      ...
      console.log('Todo list LLM response:', content)
      ...
      let todoList
      try {
        todoList = JSON.parse(cleanedContent)
      } catch (parseError) {
        console.error('JSON parse error:', parseError)
        console.error('Content that failed to parse:', cleanedContent)
        
        // Try to create a more intelligent fallback based on the user input
        const userInputLower = userInput.toLowerCase()
        if (userInputLower.includes('page') && userInputLower.includes('new')) {
          todoList = [
            {
              id: 1,
              text: 'Create newProperties directory structure',
            ...
            {
              id: 3,
              text: 'Add beautiful UI components with property cards',
              status: 'pending',
              subtasks: []
            }
          ]
          ...
          todoList = [
            {
              id: 1,
              text: `Complete user request: ${userInput}`,
              status: 'pending',
              subtasks: []
            }
          ]
  ...
  async generateSubtasks(mainTask, projectFiles, ws) {
    ...
    const systemPrompt = `You are an expert developer. Break down the main task into specific, actionable subtasks that use available tools.
...
2. Each subtask should use ONE specific tool
3. Be very specific about file paths and parameters
4. Order subtasks logically (dependencies first)
5. Focus on what's actually needed for the main task
6. Use actual file paths from the project
...
[
  ...
  },
  {
    ...
    try {
      ...
      const content = response.choices[0].message.content.trim()
      console.log('Subtasks LLM response:', content)
      ...
      let subtasks
      try {
        subtasks = JSON.parse(cleanedContent)
      } catch (parseError) {
        console.error('JSON parse error:', parseError)
        console.error('Content that failed to parse:', cleanedContent)
        
        // Create intelligent fallback based on task content
        const taskText = mainTask.text.toLowerCase()
        if (taskText.includes('directory') || taskText.includes('folder')) {
          subtasks = [
            {
              id: 1,
              text: `Create directory structure`,
              tool: 'run_command',
              parameters: { command: 'mkdir -p app/newProperties' },
              status: 'pending'
            }
          ]
        } else if (taskText.includes('page.tsx') || taskText.includes('page')) {
          subtasks = [
            {
              id: 1,
              text: `Create page.tsx file`,
              tool: 'create_file',
              parameters: { 
                path: 'app/newProperties/page.tsx',
                content: `import { Metadata } from 'next'
...
export const metadata: Metadata = {
  title: 'New Properties | Antalia Properties',
  description: 'Discover our latest luxury property listings'
}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100">
      <div className="container mx-auto px-4 py-12">
        <div className="text-center mb-16">
          <h1 className="text-5xl font-bold text-slate-800 mb-6">New Properties</h1>
          <p className="text-xl text-slate-600 max-w-3xl mx-auto leading-relaxed">
            Discover our latest and most exclusive luxury property listings
          </p>
        </div>
        ...
        } else if (taskText.includes('ui') || taskText.includes('component')) {
          subtasks = [
            {
              id: 1,
              text: `Add property cards UI`,
              tool: 'write_file',
              parameters: { 
                path: 'app/newProperties/page.tsx',
                content: `import { Metadata } from 'next'
      ...
      <div className="container mx-auto px-4 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          subtasks = [
            {
              id: 1,
              text: `Complete task: ${mainTask.text}`,
              tool: 'run_command',
              parameters: { command: 'echo "Task completed"' },
              status: 'pending'
            }
          ]
        }
